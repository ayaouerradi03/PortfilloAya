      ...
      <Container>
          ...
          {education.items.map((item, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl border border-white/12 bg-white/6 text-dragonfruit-soft transition-colors duration-500 group-hover:border-dragonfruit/40 group-hover:bg-dragonfruit/14 group-hover:text-white">
              ...
              <p className="mt-2 text-[0.98rem] font-medium text-dragonfruit-soft text-pretty">
                {item.degree}
              </p>
        ...
        <div className="mt-20">
          <div className="reveal flex items-center gap-4">
            <span className="eyebrow whitespace-nowrap">{certifications.eyebrow}</span>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <span
                      className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl border transition-colors duration-500 ${
                        earned
                          ? 'border-dragonfruit/35 bg-dragonfruit/14 text-dragonfruit-soft'
                    ...
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[0.72rem] font-semibold tracking-tight ${
                        earned
                          ? 'border-dragonfruit/35 bg-dragonfruit/12 text-dragonfruit-soft'
                          : 'border-white/14 bg-white/6 text-white/60'
                      }`}
                    >
                      {earned ? <Icon name="check" size={12} strokeWidth={2.4} /> : null}
                      {item.statusLabel}
                    </span>
                  </div>
